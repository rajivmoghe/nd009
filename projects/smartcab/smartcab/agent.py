...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5):
        ...
        #Agent inited with learning,  reset to testing, then Q-output txt doesn't work. Thus this:
        self.testing = False
        self.random_choices = 0
        self.follow_known = 0
        self.t = 0
        self.a = 0.99 #some value 0 < a < 1
    ...
    def choose_action(self, state):
        ...
        ## DONE  ##
        ###########
        # When not learning, choose a random action - this is already done above- mistake!!!.
        # The random was only for the 'non-learning' mode. Post learning, use Q values...
        # When learning, choose a random action with 'epsilon' probability
        # Otherwise, choose an action with the highest Q-value for the current state
        # Be sure that when choosing an action with highest Q-value that you randomly select between actions that "tie".
        maxQ = self.get_maxQ(state)

        if not self.testing: #we are learning, randomly amass Q values
            if self.epsilon > random.random(): #if self.epsilon > random.random(): This needs explanaiton
                action = random.choice(self.valid_actions)
                self.random_choices += 1
            else: #
                action = random.choice([act for act in self.Q[state] if self.Q[state][act] == maxQ])
                self.follow_known += 1
...
def run():
    ...
    sim = Simulator(env, display=False, update_delay=.001, log_metrics=True, optimized=True)
